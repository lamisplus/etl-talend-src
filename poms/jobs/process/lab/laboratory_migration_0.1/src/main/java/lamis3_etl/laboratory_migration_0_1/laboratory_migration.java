...
package lamis3_etl.laboratory_migration_0_1;
...
/**
 * Job: laboratory_migration Purpose: Migration of lab records<br>
 * Description: <br>
 * 
 ...
 * @status
 */
public class laboratory_migration implements TalendJob {
	...
	private final String jobName = "laboratory_migration";
		...
		public void printStackTrace() {
			...
			if (!(e instanceof TDieException)) {
				if (e instanceof TalendException) {
					...
					laboratory_migration.this.exception = e;
				}
			}
			if (!(e instanceof TalendException)) {
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(laboratory_migration.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
	...
	public void tJava_2Process(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				System.out.println("****************LABORATORY MIGRATION****************");
				System.out.println("To migrate Lab records the following tables will be affected on LAMISPlus:");
	...
	public static class visit_createStruct implements routines.system.IPersistableRow<visit_createStruct> {
		final static byte[] commonByteArrayLock_LAMIS3_ETL_laboratory_migration = new byte[0];
		static byte[] commonByteArray_LAMIS3_ETL_laboratory_migration = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_LAMIS3_ETL_laboratory_migration.length) {
					if (length < 1024 && commonByteArray_LAMIS3_ETL_laboratory_migration.length == 0) {
						commonByteArray_LAMIS3_ETL_laboratory_migration = new byte[1024];
					} else {
						commonByteArray_LAMIS3_ETL_laboratory_migration = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_LAMIS3_ETL_laboratory_migration, 0, length);
				strReturn = new String(commonByteArray_LAMIS3_ETL_laboratory_migration, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_LAMIS3_ETL_laboratory_migration) {
	...
	public static class row1Struct implements routines.system.IPersistableRow<row1Struct> {
		...
		public java.util.Date sample_date;

		public java.util.Date getSample_date() {
		...
		public void writeData(ObjectOutputStream dos) {
			try {
				...
				writeDate(this.sample_date, dos);
		...
		public String toString() {
			...
			sb.append(",archived=" + String.valueOf(archived));
			sb.append(",sample_date=" + String.valueOf(sample_date));
...
// ###############################
				...
				String dbquery_tDBInput_1 = "with lab_records as (select *\n	from laboratory, jsonb_to_recordset(lines) as x(\n	result text,\n	comment text,\n	indic"
						+ "ation text,\n	lab_test_id text\n)) \nselect l.id, l.archived,  coalesce(date_sample_collected, date_result_received) as "
						+ "sample_date, p.uuid as patient_uuid \nfrom lab_records l\n	join patient p on p.id = l.patient_id\n\n;";
				...
				try {
					...
					while (rs_tDBInput_1.next()) {
						...
						if (colQtyInRs_tDBInput_1 < 2) {
							...
							row1.archived = rs_tDBInput_1.getBoolean(2);
							if (rs_tDBInput_1.wasNull()) {
								throw new RuntimeException("Null value in non-Nullable column");
							}
						}
						if (colQtyInRs_tDBInput_1 < 3) {
							row1.sample_date = null;
						} else {

							row1.sample_date = routines.system.JDBCUtil.getDate(rs_tDBInput_1, 3);
						}
...
// # Output table : 'visit_create'
								visit_create_tmp.date_visit_start = row1.sample_date;
								visit_create_tmp.date_visit_end = row1.sample_date;
								visit_create_tmp.organisation_unit_id = 1838;
...
// Start of branch "visit_create"
						if (visit_create != null) {
							...
							pstmtUpdate_tDBOutput_1.setLong(8 + count_tDBOutput_1, visit_create.organisation_unit_id);
							...
							try {
								...
								if (updateFlag_tDBOutput_1 == 0) {
									...
									query_tDBOutput_1 = query_tDBOutput_1
											.append(String.valueOf(visit_create.patient_id))
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				String update_tDBOutput_2 = "UPDATE \"" + tableName_tDBOutput_2 + "\" SET \"" + "date_created" + "\" = "
						+ "now()"
						+ ",\"visit_id\" = ?,\"date_encounter\" = ?,\"form_code\" = ?,\"program_code\" = ?,\"created_by\" = ?,\"patient_id\" = ?,\"organisation_unit_id\" = ?,\"archived\" = ? WHERE \"visit_id\" = ? AND \"date_encounter\" = ? AND \"form_code\" = ? AND \"patient_id\" = ?";
...
// ###############################
				...
				String dbquery_tDBInput_4 = "with lab_records as (select *\n	from laboratory, jsonb_to_recordset(lines) as x(\n	result text,\n	comment text,\n	indic"
						+ "ation text,\n	lab_test_id text\n)) \nselect l.id, l.archived,  coalesce(date_sample_collected, date_result_received) as "
						+ "sample_date, p.uuid as patient_uuid \nfrom lab_records l\n	join patient p on p.id = l.patient_id\n\n;";
				...
				try {
					...
					while (rs_tDBInput_4.next()) {
						...
						if (colQtyInRs_tDBInput_4 < 2) {
							...
							row4.archived = rs_tDBInput_4.getBoolean(2);
							if (rs_tDBInput_4.wasNull()) {
								throw new RuntimeException("Null value in non-Nullable column");
							}
						}
						if (colQtyInRs_tDBInput_4 < 3) {
							row4.sample_date = null;
						} else {

							row4.sample_date = routines.system.JDBCUtil.getDate(rs_tDBInput_4, 3);
						}
						...
						if (!rejectedInnerJoin_tMap_2) { // G_TM_M_020
							...
							row5HashKey.date_visit_start = row4.sample_date == null ? null
									: new java.util.Date(row4.sample_date.getTime());
...
// # Output table : 'out1'
								...
								out1_tmp.date_encounter = row4.sample_date;
								out1_tmp.form_code = "87cb9bc7-ea0d-4c83-a70d-b57a5fb7769e";
...
// Start of branch "out1"
						if (out1 != null) {
							...
							pstmtUpdate_tDBOutput_2.setLong(9 + count_tDBOutput_2, out1.visit_id);

							if (out1.date_encounter != null) {
								pstmtUpdate_tDBOutput_2.setTimestamp(10 + count_tDBOutput_2,
										new java.sql.Timestamp(out1.date_encounter.getTime()));
							} else {
								pstmtUpdate_tDBOutput_2.setNull(10 + count_tDBOutput_2, java.sql.Types.TIMESTAMP);
							}

							if (out1.form_code == null) {
								pstmtUpdate_tDBOutput_2.setNull(11 + count_tDBOutput_2, java.sql.Types.VARCHAR);
							} else {
								pstmtUpdate_tDBOutput_2.setString(11 + count_tDBOutput_2, out1.form_code);
							}

							pstmtUpdate_tDBOutput_2.setLong(12 + count_tDBOutput_2, out1.patient_id);
							...
							try {
								...
								if (updateFlag_tDBOutput_2 == 0) {
									...
									if (out1.form_code == null) {
										...
										pstmtInsert_tDBOutput_2.setString(3, out1.form_code);
									}

									if (out1.program_code == null) {
										pstmtInsert_tDBOutput_2.setNull(4, java.sql.Types.VARCHAR);
									} else {
										pstmtInsert_tDBOutput_2.setString(4, out1.program_code);
									}

									if (out1.created_by == null) {
										pstmtInsert_tDBOutput_2.setNull(5, java.sql.Types.VARCHAR);
									} else {
										pstmtInsert_tDBOutput_2.setString(5, out1.created_by);
									}
...
// ###############################      
	...
	public static class form_data_to_createStruct
		...
		public java.util.Date date_assay;

		public java.util.Date getDate_assay() {
			return this.date_assay;
		}
		...
		public Long lab_group_id;

		public Long getLab_group_id() {
			return this.lab_group_id;
		}

		public String lab_group_name;

		public String getLab_group_name() {
			return this.lab_group_name;
		}

		public String test_name_1;

		public String getTest_name_1() {
			return this.test_name_1;
		}
		...
		public String vl_indication;

		public String getVl_indication() {
			return this.vl_indication;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_LAMIS3_ETL_laboratory_migration) {
				...
				try {
					...
					this.labno = readString(dis);

					this.date_assay = readDate(dis);
					...
					length = dis.readByte();
					if (length == -1) {
						this.lab_group_id = null;
					} else {
						this.lab_group_id = dis.readLong();
					}

					this.lab_group_name = readString(dis);

					this.test_name_1 = readString(dis);
					...
					this.vl_indication = readString(dis);
		...
		public void writeData(ObjectOutputStream dos) {
			try {
				...
				writeString(this.labno, dos);

				// java.util.Date

				writeDate(this.date_assay, dos);
				...
				if (this.lab_group_id == null) {
					dos.writeByte(-1);
				} else {
					dos.writeByte(0);
					dos.writeLong(this.lab_group_id);
				}

				// String

				writeString(this.lab_group_name, dos);

				// String

				writeString(this.test_name_1, dos);
				...
				writeString(this.vl_indication, dos);
		...
		public String toString() {
			...
			sb.append(super.toString());
			sb.append("[");
			sb.append("labno=" + labno);
			sb.append(",date_assay=" + String.valueOf(date_assay));
			sb.append(",createdby=" + createdby);
			sb.append(",id=" + String.valueOf(id));
			sb.append(",lab_group_id=" + String.valueOf(lab_group_id));
			sb.append(",lab_group_name=" + lab_group_name);
			sb.append(",test_name_1=" + test_name_1);
			...
			sb.append(",patient_id=" + String.valueOf(patient_id));
			sb.append(",vl_indication=" + vl_indication);
	...
	public static class row6Struct implements routines.system.IPersistableRow<row6Struct> {
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_LAMIS3_ETL_laboratory_migration) {
				...
				try {
					...
					this.lab_test_id = readString(dis);

					this.sample_date = readDate(dis);

					this.test_name = readString(dis);
		...
		public void writeData(ObjectOutputStream dos) {
			try {
				...
				writeString(this.lab_test_id, dos);
				...
				writeString(this.test_name, dos);
		...
		public String toString() {
			...
			sb.append(",comment=" + comment);
			sb.append(",indication=" + indication);
			sb.append(",lab_test_id=" + lab_test_id);
			sb.append(",sample_date=" + String.valueOf(sample_date));
			sb.append(",test_name=" + test_name);
			sb.append(",patient_uuid=" + patient_uuid);
...
// ###############################
				...
				fileName_tFileOutputDelimited_1 = (new java.io.File("laboratory_not_migrated.csv")).getAbsolutePath()
						.replace("\\", "/");
				...
				if (filetFileOutputDelimited_1.length() == 0) {
					...
					headColutFileOutputDelimited_1[2] = "facility_id";
					headColutFileOutputDelimited_1[3] = "lines";
					headColutFileOutputDelimited_1[4] = "date_result_received";
					headColutFileOutputDelimited_1[5] = "date_sample_collected";
					headColutFileOutputDelimited_1[6] = "reason";
...
// ###############################
				...
				String dbquery_tDBInput_6 = "with lab_records as (select *\n	from laboratory, jsonb_to_recordset(lines) as x(\n	result text,\n	comment text,\n	indic"
						+ "ation text,\n	lab_test_id text\n)) \nselect l.id, l.patient_id, l.facility_id, l.date_sample_collected,l.date_result_rec"
						+ "eived, l.labno, l.uuid, l.date_assay, l.lines, l.result, l.comment, l.indication, l.lab_test_id,  coalesce(date_sample_c"
						+ "ollected, date_result_received) as sample_date, upper(lt.description) as test_name, p.uuid as patient_uuid, upper(f.name"
						+ ") as facilityName, '' as createdBy \nfrom lab_records l\njoin lab_test lt on lt.id = l.lab_test_id::int\n	join patient p"
				...
				try {
					...
					while (rs_tDBInput_6.next()) {
						...
						if (colQtyInRs_tDBInput_6 < 2) {
							...
							row6.patient_id = rs_tDBInput_6.getLong(2);
							if (rs_tDBInput_6.wasNull()) {
								throw new RuntimeException("Null value in non-Nullable column");
							}
						}
						if (colQtyInRs_tDBInput_6 < 3) {
							...
							row6.facility_id = rs_tDBInput_6.getLong(3);
							if (rs_tDBInput_6.wasNull()) {
								throw new RuntimeException("Null value in non-Nullable column");
							}
						}
						if (colQtyInRs_tDBInput_6 < 4) {
							row6.date_sample_collected = null;
						} else {

							row6.date_sample_collected = routines.system.JDBCUtil.getDate(rs_tDBInput_6, 4);
						}
						if (colQtyInRs_tDBInput_6 < 5) {
							row6.date_result_received = null;
						} else {

							row6.date_result_received = routines.system.JDBCUtil.getDate(rs_tDBInput_6, 5);
						}
						if (colQtyInRs_tDBInput_6 < 6) {
							row6.labno = null;
						} else {

							row6.labno = routines.system.JDBCUtil.getString(rs_tDBInput_6, 6, false);
						}
						if (colQtyInRs_tDBInput_6 < 7) {
							row6.uuid = null;
						} else {

							row6.uuid = routines.system.JDBCUtil.getString(rs_tDBInput_6, 7, false);
						}
						if (colQtyInRs_tDBInput_6 < 8) {
							row6.date_assay = null;
						} else {

							row6.date_assay = routines.system.JDBCUtil.getDate(rs_tDBInput_6, 8);
						}
						if (colQtyInRs_tDBInput_6 < 9) {
							row6.lines = null;
						} else {

							row6.lines = routines.system.JDBCUtil.getString(rs_tDBInput_6, 9, false);
						}
						if (colQtyInRs_tDBInput_6 < 10) {
							row6.result = null;
						} else {

							row6.result = routines.system.JDBCUtil.getString(rs_tDBInput_6, 10, false);
						}
						if (colQtyInRs_tDBInput_6 < 11) {
							row6.comment = null;
						} else {

							row6.comment = routines.system.JDBCUtil.getString(rs_tDBInput_6, 11, false);
						}
						if (colQtyInRs_tDBInput_6 < 12) {
							row6.indication = null;
						} else {

							row6.indication = routines.system.JDBCUtil.getString(rs_tDBInput_6, 12, false);
						}
						if (colQtyInRs_tDBInput_6 < 13) {
							row6.lab_test_id = null;
						} else {

							row6.lab_test_id = routines.system.JDBCUtil.getString(rs_tDBInput_6, 13, false);
						}
						if (colQtyInRs_tDBInput_6 < 14) {
							row6.sample_date = null;
						} else {

							row6.sample_date = routines.system.JDBCUtil.getDate(rs_tDBInput_6, 14);
						}
						if (colQtyInRs_tDBInput_6 < 15) {
							row6.test_name = null;
						} else {

							row6.test_name = routines.system.JDBCUtil.getString(rs_tDBInput_6, 15, false);
						}
						if (colQtyInRs_tDBInput_6 < 16) {
							row6.patient_uuid = null;
						} else {

							row6.patient_uuid = routines.system.JDBCUtil.getString(rs_tDBInput_6, 16, false);
						}
						if (colQtyInRs_tDBInput_6 < 17) {
							row6.facilityname = null;
						} else {

							row6.facilityname = routines.system.JDBCUtil.getString(rs_tDBInput_6, 17, false);
						}
						if (colQtyInRs_tDBInput_6 < 18) {
							row6.createdby = null;
						} else {

							row6.createdby = routines.system.JDBCUtil.getString(rs_tDBInput_6, 18, false);
						}
						...
						if (!rejectedInnerJoin_tMap_3) { // G_TM_M_020
							...
							row7HashKey.date_encounter = row6.sample_date == null ? null
									: new java.util.Date(row6.sample_date.getTime());
						...
						if (!rejectedInnerJoin_tMap_3) { // G_TM_M_020
							...
							row8HashKey.test_name = row6.test_name;
						...
						if (tHash_Lookup_row8 != null && tHash_Lookup_row8.getCount(row8HashKey) > 1) { // G 071
							...
							// and it contains more one result from keys : row8.test_name = '" +
							// row8HashKey.test_name + "'");
						} // G 071
...
// # Output table : 'form_data_to_create'
									form_data_to_create_tmp.labno = row6.labno;
									form_data_to_create_tmp.date_assay = row6.date_assay;
									form_data_to_create_tmp.createdby = row6.createdby;
									form_data_to_create_tmp.id = row8.id;
									form_data_to_create_tmp.lab_group_id = row8.lab_group_id;
									form_data_to_create_tmp.lab_group_name = row8.lab_group_name;
									form_data_to_create_tmp.test_name_1 = row6.test_name;
									form_data_to_create_tmp.date_result_received = row6.date_result_received;
									form_data_to_create_tmp.sample_date = row6.sample_date;
									form_data_to_create_tmp.name = row8.name;
									form_data_to_create_tmp.result = row6.result;
									form_data_to_create_tmp.comment = row6.comment;
									form_data_to_create_tmp.unit_measurement = row8.unit_measurement;
									...
									form_data_to_create_tmp.vl_indication = row9.vl_indication;
									form_data_to_create_tmp.indication = row6.indication;
...
// # Filter conditions 
								if (rejectedInnerJoin_tMap_3) {
									form_data_reject_tmp.id = row6.id;
									form_data_reject_tmp.patient_id = row6.patient_id;
									form_data_reject_tmp.facility_id = row6.facility_id;
									form_data_reject_tmp.lines = row6.lines;
									form_data_reject_tmp.date_result_received = row6.date_result_received;
									form_data_reject_tmp.date_sample_collected = row6.date_sample_collected;
									form_data_reject_tmp.reason = "Lab test does not exist on LAMISPlus";
...
//setting the default json values in the form data object
								JSONObject json = new JSONObject(
										"{\"comment\":\"\",\"user_id\":\"\",\"lab_number\":\"\",\"sample_type\":[],\"order_priority\":{\"id\":295,\"code\":\"6fca9e78-3305-43a5-a45e-dddb92aa4e6e\",\"display\":\"NORMAL\",\"language\":\"en\",\"codesetGroup\":\"TEST_ORDER_PRIORITY\"},\"sample_order_time\":\"\",\"sample_verified_by\":\"\",\"sample_collected_by\":\"\",\"date_sample_verified\":\"\",\"time_sample_Verified\":\"\",\"lab_test_order_status\":5,\"time_sample_collected\":\"\",\"sample_collection_mode\":[],\"comment_sample_verified\":\"\",\"comment_sample_collected\":\"\"}");
								json.put("comment", form_data_to_create.comment);
								json.put("lab_number", form_data_to_create.labno);
								json.put("patient_id", form_data_to_create.patient_id);
								json.put("description", form_data_to_create.name);
								json.put("lab_test_id", form_data_to_create.id);
								json.put("lab_test_group", form_data_to_create.lab_group_name);
								json.put("lab_test_group_id", form_data_to_create.lab_group_id);

								json.put("unit_measurement", form_data_to_create.unit_measurement);
								json.put("sample_order_date",
										form_data_to_create.sample_date != null
												? formatter.format(form_data_to_create.sample_date)
												: JSONObject.NULL);
								json.put("date_result_reported",
										form_data_to_create.date_result_received != null
												? formatter.format(form_data_to_create.date_result_received)
												: JSONObject.NULL);
								json.put("date_sample_collected", "");
								if (form_data_to_create.indication != null) {
									JSONArray vlIndications = new JSONArray(form_data_to_create.vl_indication);
									String status = form_data_to_create.indication;
									status = status.equalsIgnoreCase("Routine Monitoring") ? "Routine (every 12 months)"
											: status;
									// status = status.equalsIgnoreCase("Targeted Monitoring") ? "Routine (every 12
									// months)" : status;

									JSONObject item = null;
									for (int x = 0; x < vlIndications.length(); x++) {
										if (vlIndications.getJSONObject(x).getString("display")
												.equalsIgnoreCase(status)) {
											item = vlIndications.getJSONObject(x);
											break;
										}
									}

									if (item != null) {
										json.put("viral_load_indication", item);
									} else {
										context.application_codeset_not_present = context.application_codeset_not_present
												...
												+ "viral_load_indication - " + status + "'";
									}

								}

								JSONObject sampleType = new JSONObject();
								sampleType.put("id", 186);
								sampleType.put("code", "ee0947");
								sampleType.put("display", "Blood");
								sampleType.put("language", "en");
								sampleType.put("codesetGroup", "SAMPLE_TYPE");
								json.put("sample_type", sampleType);

								JSONArray array = new JSONArray();
								JSONObject item = new JSONObject();
								item.put("date_asseyed", "");
								item.put("date_result_reported",
										form_data_to_create.date_result_received != null
												? formatter.format(form_data_to_create.date_result_received)
												: JSONObject.NULL);
								item.put("time_result_reported", "");
								item.put("result_reported", form_data_to_create.result);
								item.put("result_reported_by", "");
								item.put("comment_result_reported", form_data_to_create.comment);
								array.put(item);

								json.put("reported_result", array);
								...
								if (!rejectedInnerJoin_tMap_4) { // G_TM_M_020
									...
									row11HashKey.lab_test_id = row10.id;
								...
								if (tHash_Lookup_row11 != null && tHash_Lookup_row11.getCount(row11HashKey) > 1) { // G
									...
									// row11HashKey.encounter_id + "', row11.lab_test_id = '" +
									// row11HashKey.lab_test_id + "'");
								} // G 071
...
// Start of branch "form_data_reject"
							if (form_data_reject != null) {
								...
								rowtFileOutputDelimited_1[2] = String.valueOf(form_data_reject.facility_id);
								rowtFileOutputDelimited_1[3] = form_data_reject.lines == null ? null
										: form_data_reject.lines;
								rowtFileOutputDelimited_1[4] = form_data_reject.date_result_received == null ? null
										: FormatterUtils.format_Date(form_data_reject.date_result_received,
												"yyyy-MM-dd");
								rowtFileOutputDelimited_1[5] = form_data_reject.date_sample_collected == null ? null
										: FormatterUtils.format_Date(form_data_reject.date_sample_collected,
												"yyyy-MM-dd");
								rowtFileOutputDelimited_1[6] = form_data_reject.reason == null ? null
...
// ###############################      
	...
	public void tJava_6Process(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				System.out
						.println("*******************LABORATORY MIGRATION COMPLETED*********************************");

				System.out.println("*****************************************************");
				System.out.println("Check ./laboratory_not_migrated.csv for records that did not migrate. ");
	...
	public void tDBInput_3Process(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				// linked node: tMap_1 - inputs:(row1,row3) outputs:(visit_create)
	...
	public void tDBInput_7Process(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				// linked node: tMap_3 - inputs:(row6,row7,row8,row9)
				...
				String dbquery_tDBInput_7 = "select v.date_encounter, p.uuid, v.id, p.id as patient_id, o.id as facilityId\nfrom encounter v\njoin patient p on v.pa"
						+ "tient_id = p.id\nleft join organisation_unit o on o.id = v.organisation_unit_id\nwhere v.form_code = '87cb9bc7-ea0d-4c83"
						+ "-a70d-b57a5fb7769e'";
	...
	public static class row8Struct implements routines.system.IPersistableComparableLookupRow<row8Struct> {
		...
		public int hashCode() {
			if (this.hashCodeDirty) {
				...
				result = prime * result + ((this.test_name == null) ? 0 : this.test_name.hashCode());
		...
		public boolean equals(Object obj) {
			...
			} else if (!this.test_name.equals(other.test_name))
		...
		public void copyDataTo(row8Struct other) {
			...
			other.test_name = this.test_name;
			other.name = this.name;
			other.id = this.id;
			other.lab_group_id = this.lab_group_id;
			other.lab_group_name = this.lab_group_name;
		...
		public void readValuesData(DataInputStream dis, ObjectInputStream ois) {
			try {
				...
				this.unit_measurement = readString(dis, ois);
				...
				this.lab_group_name = readString(dis, ois);
		...
		public void writeValuesData(DataOutputStream dos, ObjectOutputStream oos) {
			try {
				...
				writeString(this.unit_measurement, dos, oos);
				...
				writeString(this.lab_group_name, dos, oos);
		...
		public int compareTo(row8Struct other) {
			...
			returnValue = checkNullsAndCompare(this.test_name, other.test_name);
			if (returnValue != 0) {
				return returnValue;
			}
	...
	public void tDBInput_8Process(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				String dbquery_tDBInput_8 = "select l.unit_measurement as unit_measurement, upper(l.name) as test_name, l.name as name, l.id, lt.id as lab_group_id,"
						+ " lt.name as lab_group_name\nfrom lab_test l\nleft join lab_test_group lt on lt.id = l.lab_test_group_id\n";
				...
				try {
					...
					while (rs_tDBInput_8.next()) {
						...
						if (colQtyInRs_tDBInput_8 < 1) {
							row8.unit_measurement = null;
						} else {

							row8.unit_measurement = routines.system.JDBCUtil.getString(rs_tDBInput_8, 1, false);
						}
						if (colQtyInRs_tDBInput_8 < 2) {
							row8.test_name = null;
						} else {

							row8.test_name = routines.system.JDBCUtil.getString(rs_tDBInput_8, 2, false);
						}
						if (colQtyInRs_tDBInput_8 < 3) {
							row8.name = null;
						} else {

							row8.name = routines.system.JDBCUtil.getString(rs_tDBInput_8, 3, false);
						}
						if (colQtyInRs_tDBInput_8 < 4) {
							row8.id = 0;
						} else {

							row8.id = rs_tDBInput_8.getLong(4);
							if (rs_tDBInput_8.wasNull()) {
								throw new RuntimeException("Null value in non-Nullable column");
							}
						}
						if (colQtyInRs_tDBInput_8 < 5) {
							row8.lab_group_id = null;
						} else {

							row8.lab_group_id = rs_tDBInput_8.getLong(5);
							if (rs_tDBInput_8.wasNull()) {
								row8.lab_group_id = null;
							}
						}
						if (colQtyInRs_tDBInput_8 < 6) {
							row8.lab_group_name = null;
						} else {

							row8.lab_group_name = routines.system.JDBCUtil.getString(rs_tDBInput_8, 6, false);
						}
						...
						row8_HashRow.unit_measurement = row8.unit_measurement;

						row8_HashRow.test_name = row8.test_name;

						row8_HashRow.name = row8.name;

						row8_HashRow.id = row8.id;

						row8_HashRow.lab_group_id = row8.lab_group_id;

						row8_HashRow.lab_group_name = row8.lab_group_name;
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				String dbquery_tDBInput_9 = "select array_to_json(array(select json_build_object('id', id, 'display',display,'codeset_group',codeset_group,'code',co"
						+ "de,'language',language) as json_object \n from application_codeset where codeset_group in ('VIRAL_LOAD_INDICATION'))) as"
						+ " vl_indication";
				...
				try {
					...
					while (rs_tDBInput_9.next()) {
						...
						if (colQtyInRs_tDBInput_9 < 1) {
							row9.vl_indication = null;
						} else {

							row9.vl_indication = routines.system.JDBCUtil.getString(rs_tDBInput_9, 1, false);
						}
						...
						row9_HashRow.vl_indication = row9.vl_indication;
	...
	public static class row11Struct implements routines.system.IPersistableComparableLookupRow<row11Struct> {
		...
		public Long lab_test_id;

		public Long getLab_test_id() {
			return this.lab_test_id;
		}
		...
		public int hashCode() {
			if (this.hashCodeDirty) {
				...
				result = prime * result + ((this.lab_test_id == null) ? 0 : this.lab_test_id.hashCode());
		...
		public boolean equals(Object obj) {
			...
			if (this.lab_test_id == null) {
				if (other.lab_test_id != null)
					return false;

			} else if (!this.lab_test_id.equals(other.lab_test_id))
		...
		public void writeKeysData(ObjectOutputStream dos) {
			try {
				...
				if (this.lab_test_id == null) {
					...
					dos.writeByte(0);
					dos.writeLong(this.lab_test_id);
				}
		...
		public int compareTo(row11Struct other) {
			...
			returnValue = checkNullsAndCompare(this.lab_test_id, other.lab_test_id);
			if (returnValue != 0) {
				return returnValue;
			}
	...
	public void tDBInput_10Process(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				String dbquery_tDBInput_10 = "select f.id, f.encounter_id, (f.data ->> 'lab_test_id')::int as lab_test_id from form_data f join encounter e on f.enco"
						+ "unter_id = e.id and e.form_code = '87cb9bc7-ea0d-4c83-a70d-b57a5fb7769e'";
				...
				try {
					...
					while (rs_tDBInput_10.next()) {
						...
						if (colQtyInRs_tDBInput_10 < 3) {
							row11.lab_test_id = null;
						} else {

							row11.lab_test_id = rs_tDBInput_10.getLong(3);
							if (rs_tDBInput_10.wasNull()) {
								row11.lab_test_id = null;
							}
						}
						...
						row11_HashRow.lab_test_id = row11.lab_test_id;
	...
	public static void main(String[] args) {
		final laboratory_migration laboratory_migrationClass = new laboratory_migration();

		int exitCode = laboratory_migrationClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = laboratory_migration.class.getClassLoader()
					.getResourceAsStream("lamis3_etl/laboratory_migration_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = laboratory_migration.class.getClassLoader()
		...
		if (false) {
			System.out.println(
					(endUsedMemory - startUsedMemory) + " bytes memory increase when running : laboratory_migration");
		}
...
/************************************************************************************************
 * 361877 characters generated by Talend Open Studio for Big Data on the May 31,
 * 2021 11:04:43 AM WAT
 ************************************************************************************************/