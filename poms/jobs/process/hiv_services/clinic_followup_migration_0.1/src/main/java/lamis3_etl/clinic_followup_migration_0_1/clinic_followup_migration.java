...
package lamis3_etl.clinic_followup_migration_0_1;
...
/**
 * Job: clinic_followup_migration Purpose: To migrate HIV enrollment records<br>
 * Description: <br>
 * 
 ...
 * @status
 */
public class clinic_followup_migration implements TalendJob {
	...
	private final String jobName = "clinic_followup_migration";
		...
		public void printStackTrace() {
			...
			if (!(e instanceof TDieException)) {
				if (e instanceof TalendException) {
					...
					clinic_followup_migration.this.exception = e;
				}
			}
			if (!(e instanceof TalendException)) {
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(clinic_followup_migration.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
	...
	public static class to_updateStruct implements routines.system.IPersistableRow<to_updateStruct> {
		final static byte[] commonByteArrayLock_LAMIS3_ETL_clinic_followup_migration = new byte[0];
		static byte[] commonByteArray_LAMIS3_ETL_clinic_followup_migration = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_LAMIS3_ETL_clinic_followup_migration.length) {
					if (length < 1024 && commonByteArray_LAMIS3_ETL_clinic_followup_migration.length == 0) {
						commonByteArray_LAMIS3_ETL_clinic_followup_migration = new byte[1024];
					} else {
						commonByteArray_LAMIS3_ETL_clinic_followup_migration = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_LAMIS3_ETL_clinic_followup_migration, 0, length);
				strReturn = new String(commonByteArray_LAMIS3_ETL_clinic_followup_migration, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_LAMIS3_ETL_clinic_followup_migration) {
...
// ###############################
				...
				fileName_tFileOutputDelimited_1 = (new java.io.File("clinic_not_migrated.csv")).getAbsolutePath()
						.replace("\\", "/");
				...
				try {
					...
					while (rs_tDBInput_6.next()) {
						...
						if (!rejectedInnerJoin_tMap_3) { // G_TM_M_020
							...
							row2HashKey.composition = row6.regimen
						...
						if (tHash_Lookup_row2 != null && tHash_Lookup_row2.getCount(row2HashKey) > 1) { // G 071
							...
							// and it contains more one result from keys : row2.composition = '" +
							// row2HashKey.composition + "'");
						} // G 071
...
// Start of branch "form_data_to_create"
									if (form_data_to_create != null) {
										...
										JSONArray clinic = new JSONArray(
												form_data_to_create.clinic_application_codeset);

										JSONObject json = new JSONObject();
										json.put("cd4", form_data_to_create.cd4);
										json.put("cd4p", form_data_to_create.cd4p);
										json.put("date_art_start",
												form_data_to_create.date_visit != null
														? formatter.format(form_data_to_create.date_visit)
														: JSONObject.NULL);
										json.put("date_enrollment",
												form_data_to_create.date_visit != null
														? formatter.format(form_data_to_create.date_visit)
														: JSONObject.NULL);
										...
										if (form_data_to_create.func_status != null
												&& form_data_to_create.func_status != "") {
											...
											for (int x = 0; x < func.length(); x++) {
												if (func.getJSONObject(x).getString("display")
										...
										if (form_data_to_create.clinic_stage != null
												&& form_data_to_create.clinic_stage != "") {
											String status = form_data_to_create.clinic_stage;
											// status = status.equalsIgnoreCase("HIV_PLUS_NON_ART") ? "HIV+ non ART" :
											// status;
											JSONObject item = null;
											for (int x = 0; x < clinic.length(); x++) {
												if (clinic.getJSONObject(x).getString("display")
														.equalsIgnoreCase(status)) {
...
// Start of branch "form_data_reject"
									if (form_data_reject != null) {
										...
										rowtFileOutputDelimited_1[1] = form_data_reject.date_visit == null ? null
												: FormatterUtils.format_Date(form_data_reject.date_visit, "yyyy-MM-dd");
										rowtFileOutputDelimited_1[2] = form_data_reject.clinic_uuid == null ? null
												: form_data_reject.clinic_uuid;
										rowtFileOutputDelimited_1[3] = form_data_reject.regimen == null ? null
												: form_data_reject.regimen;
										rowtFileOutputDelimited_1[4] = form_data_reject.reason == null ? null
...
//System.out.println("Check ./clinic_not_migrated.csv for records that did not migrate. ");
	...
	public void tLibraryLoad_2Process(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		try {
			...
			if (resumeEntryMethodName == null || globalResumeTicket) {
				resumeUtil.addLog("CHECKPOINT", "CONNECTION:SUBJOB_OK:tLibraryLoad_2:OnSubjobOk1", "",
						Thread.currentThread().getId() + "", "", "", "", "", "");
			}
			...
			if (resumeEntryMethodName == null || globalResumeTicket) {
				resumeUtil.addLog("CHECKPOINT", "CONNECTION:SUBJOB_OK:tLibraryLoad_2:OnSubjobOk2", "",
						Thread.currentThread().getId() + "", "", "", "", "", "");
			}
	...
	public static class copyOfform_data_to_createStruct
			implements routines.system.IPersistableRow<copyOfform_data_to_createStruct> {
		final static byte[] commonByteArrayLock_LAMIS3_ETL_clinic_followup_migration = new byte[0];
		static byte[] commonByteArray_LAMIS3_ETL_clinic_followup_migration = new byte[0];
	...
	public static class copyOfform_data_rejectStruct
			implements routines.system.IPersistableRow<copyOfform_data_rejectStruct> {
		final static byte[] commonByteArrayLock_LAMIS3_ETL_clinic_followup_migration = new byte[0];
		static byte[] commonByteArray_LAMIS3_ETL_clinic_followup_migration = new byte[0];
...
// ###############################
				...
				fileName_tFileOutputDelimited_2 = (new java.io.File("clinic_not_migrated.csv")).getAbsolutePath()
						.replace("\\", "/");
...
// ###############################
				...
				String url_tDBInput_12 = "jdbc:postgresql://" + context.LAMIS3_Server + ":" + context.LAMIS3_Port + "/"
						+ context.LAMIS3_Database + "?" + context.LAMIS3_AdditionalParams;
				...
				try {
					...
					while (rs_tDBInput_12.next()) {
						...
						if (!rejectedInnerJoin_tMap_5) { // G_TM_M_020
							...
							row14HashKey.date_encounter = row13.date_visit == null ? null
									: new java.util.Date(row13.date_visit.getTime());
						...
						if (tHash_Lookup_row14 != null && tHash_Lookup_row14.getCount(row14HashKey) > 1) { // G 071
							...
							// System.out.println("WARNING: UNIQUE MATCH is configured for the lookup
							// 'row14' and it contains more one result from keys : row14.date_encounter = '"
							// + row14HashKey.date_encounter + "', row14.uuid = '" + row14HashKey.uuid +
							// "'");
						} // G 071
						...
						if (!rejectedInnerJoin_tMap_5) { // G_TM_M_020
							...
							row16HashKey.composition = row13.regimen == null ? null
									: row13.regimen
											.replace("Lamivudine+Nevirapine+Zidovudine",
													"Zidovudine+Lamivudine+Nevirapine")
											.replace("Zidovudine+Lamivudine+Nevarapin",
													"Zidovudine+Lamivudine+Nevirapine")
											.replace("Efavirenz+Emtricitabine+Tenofovir",
													"Tenofovir+Emtricitabine+Efavirenz")
											.replace("Efavirenz+Lamivudine+Zidovudine",
													"Zidovudine+Lamivudine+Efavirenz")
											.replace("Emtricitabine+Tenofovir+Lopinavir/Ritonavir",
													"Tenofovir+Emtricitabine+Lopinavir/Ritonavir")
											.replace("Emtricitabine+Tenofovir+Nevirapine",
													"Tenofovir+Emtricitabine+Nevirapine")
											.replace("Efavirenz+Lamivudine+Stavudine",
						...
						if (tHash_Lookup_row16 != null && tHash_Lookup_row16.getCount(row16HashKey) > 1) { // G 071
							...
							// System.out.println("WARNING: UNIQUE MATCH is configured for the lookup
							// 'row16' and it contains more one result from keys : row16.composition = '" +
							// row16HashKey.composition + "'");
						} // G 071
						...
						while ((tHash_Lookup_row17 != null && tHash_Lookup_row17.hasNext()) || forceLooprow17) { // G_TM_M_043
							...
							while ((tHash_Lookup_row18 != null && tHash_Lookup_row18.hasNext()) || forceLooprow18) { // G_TM_M_043
								...
								while ((tHash_Lookup_row19 != null && tHash_Lookup_row19.hasNext()) || forceLooprow19) { // G_TM_M_043
									...
									while ((tHash_Lookup_row21 != null && tHash_Lookup_row21.hasNext())
											|| forceLooprow21) { // G_TM_M_043
										...
										if (copyOfform_data_to_create != null) {
											...
											if (execStat) {
												runStat.updateStatOnConnection(iterateId, 1, 1,
														"copyOfform_data_to_create");
											}

											SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd");

											JSONArray tb = new JSONArray(
													copyOfform_data_to_create.tb_application_codeset);
											JSONArray func = new JSONArray(
													copyOfform_data_to_create.func_application_codeset);
											JSONArray clinic = new JSONArray(
													copyOfform_data_to_create.clinic_application_codeset);
											JSONArray adherence = new JSONArray(
													copyOfform_data_to_create.adherence_application_codeset);

											JSONObject json = new JSONObject();
											json.put("cd4", copyOfform_data_to_create.cd4);
											json.put("cd4p", copyOfform_data_to_create.cd4p);
											json.put("date_visit",
													copyOfform_data_to_create.date_visit != null
															? formatter.format(copyOfform_data_to_create.date_visit)
															: JSONObject.NULL);
											json.put("next_appointment",
													copyOfform_data_to_create.next_appointment != null
															? formatter
																	.format(copyOfform_data_to_create.next_appointment)
															: JSONObject.NULL);
											...
											if (copyOfform_data_to_create.bp != null
													&& copyOfform_data_to_create.bp != "") {
											...
											if (copyOfform_data_to_create.breastfeeding != null
													&& copyOfform_data_to_create.breastfeeding == true) {
												json.put("pregnancy_status", new JSONObject(
														"{\"id\":75,\"display\":\"Breastfeeding\",\"codeset_group\":\"PREGANACY_STATUS\",\"code\":\"5240c509-a32e-4343-bc85-247f03671b9b\",\"language\":\"en\"}"));
											}

											if (copyOfform_data_to_create.func_status != null
													&& copyOfform_data_to_create.func_status != "") {
											...
											if (copyOfform_data_to_create.clinic_stage != null
													&& copyOfform_data_to_create.clinic_stage != "") {
											...
											if (copyOfform_data_to_create.tb_status != null
													&& copyOfform_data_to_create.tb_status.trim() != "") {
											...
											if (copyOfform_data_to_create.adherence_level != null
													&& copyOfform_data_to_create.adherence_level != "") {
												...
												for (int x = 0; x < adherence.length(); x++) {
													if (adherence.getJSONObject(x).getString("display")
															.equalsIgnoreCase(status)) {
...
//row15.id = copyOfform_data_to_create.id;
											...
											if (tHash_Lookup_row20 != null
													&& tHash_Lookup_row20.getCount(row20HashKey) > 1) { // G 071

												// System.out.println("WARNING: UNIQUE MATCH is configured for the
												// lookup 'row20' and it contains more one result from keys :
												// row20.encounter_id = '" + row20HashKey.encounter_id + "',
												// row20.backlog_uuid = '" + row20HashKey.backlog_uuid + "'");
											} // G 071
...
// Start of branch "copyOfform_data_reject"
										if (copyOfform_data_reject != null) {
											...
											if (execStat) {
												runStat.updateStatOnConnection(iterateId, 1, 1,
														"copyOfform_data_reject");
											}

											String[] rowtFileOutputDelimited_2 = new String[5];
											rowtFileOutputDelimited_2[0] = copyOfform_data_reject.patient_uuid == null
													? null
													: copyOfform_data_reject.patient_uuid;
											rowtFileOutputDelimited_2[1] = copyOfform_data_reject.date_visit == null
													? null
													: FormatterUtils.format_Date(copyOfform_data_reject.date_visit,
															"yyyy-MM-dd");
											rowtFileOutputDelimited_2[2] = copyOfform_data_reject.clinic_uuid == null
													? null
													: copyOfform_data_reject.clinic_uuid;
											rowtFileOutputDelimited_2[3] = copyOfform_data_reject.regimen == null ? null
													: copyOfform_data_reject.regimen;
											rowtFileOutputDelimited_2[4] = copyOfform_data_reject.reason == null ? null
													: copyOfform_data_reject.reason;
...
// ###############################      
				...
				try {
					...
					while (rs_tDBInput_9.next()) {
						...
						if (colQtyInRs_tDBInput_9 < 1) {
							...
							row9.clinic_application_codeset = routines.system.JDBCUtil.getString(rs_tDBInput_9, 1,
									false);
						}
				...
				try {
					...
					while (rs_tDBInput_11.next()) {
						...
						if (colQtyInRs_tDBInput_11 < 1) {
							...
							row12.func_application_codeset = routines.system.JDBCUtil.getString(rs_tDBInput_11, 1,
									false);
						}
	...
	public void tDBInput_13Process(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				// source node:tDBInput_13 - inputs:(after_tDBInput_12) outputs:(row14,row14) |
				// target node:tAdvancedHash_row14 - inputs:(row14) outputs:()
				// linked node: tMap_5 - inputs:(row13,row14,row16,row17,row18,row19,row21)
				// outputs:(copyOfform_data_to_create,copyOfform_data_reject)
	...
	public void tDBInput_14Process(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				// source node:tDBInput_14 - inputs:(after_tDBInput_12) outputs:(row16,row16) |
				...
				String url_tDBInput_14 = "jdbc:postgresql://" + context.LAMISPlus_Server + ":" + context.LAMISPlus_Port
						+ "/" + context.LAMISPlus_Database + "?" + context.LAMISPlus_AdditionalParams;
	...
	public void tDBInput_15Process(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				// source node:tDBInput_15 - inputs:(after_tDBInput_12) outputs:(row17,row17) |
				// target node:tAdvancedHash_row17 - inputs:(row17) outputs:()
				// linked node: tMap_5 - inputs:(row13,row14,row16,row17,row18,row19,row21)
				// outputs:(copyOfform_data_to_create,copyOfform_data_reject)

				org.talend.designer.components.lookup.common.ICommonLookup.MATCHING_MODE matchingModeEnum_row17 = org.talend.designer.components.lookup.common.ICommonLookup.MATCHING_MODE.ALL_ROWS;
				...
				String url_tDBInput_15 = "jdbc:postgresql://" + context.LAMISPlus_Server + ":" + context.LAMISPlus_Port
						+ "/" + context.LAMISPlus_Database + "?" + context.LAMISPlus_AdditionalParams;
	...
	public void tDBInput_16Process(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				// source node:tDBInput_16 - inputs:(after_tDBInput_12) outputs:(row18,row18) |
				// target node:tAdvancedHash_row18 - inputs:(row18) outputs:()
				// linked node: tMap_5 - inputs:(row13,row14,row16,row17,row18,row19,row21)
				// outputs:(copyOfform_data_to_create,copyOfform_data_reject)

				org.talend.designer.components.lookup.common.ICommonLookup.MATCHING_MODE matchingModeEnum_row18 = org.talend.designer.components.lookup.common.ICommonLookup.MATCHING_MODE.ALL_ROWS;
				...
				String url_tDBInput_16 = "jdbc:postgresql://" + context.LAMISPlus_Server + ":" + context.LAMISPlus_Port
						+ "/" + context.LAMISPlus_Database + "?" + context.LAMISPlus_AdditionalParams;
				...
				try {
					...
					while (rs_tDBInput_16.next()) {
						...
						if (colQtyInRs_tDBInput_16 < 1) {
							...
							row18.clinic_application_codeset = routines.system.JDBCUtil.getString(rs_tDBInput_16, 1,
									false);
						}
	...
	public void tDBInput_17Process(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				// source node:tDBInput_17 - inputs:(after_tDBInput_12) outputs:(row19,row19) |
				// target node:tAdvancedHash_row19 - inputs:(row19) outputs:()
				// linked node: tMap_5 - inputs:(row13,row14,row16,row17,row18,row19,row21)
				// outputs:(copyOfform_data_to_create,copyOfform_data_reject)

				org.talend.designer.components.lookup.common.ICommonLookup.MATCHING_MODE matchingModeEnum_row19 = org.talend.designer.components.lookup.common.ICommonLookup.MATCHING_MODE.ALL_ROWS;
				...
				String url_tDBInput_17 = "jdbc:postgresql://" + context.LAMISPlus_Server + ":" + context.LAMISPlus_Port
						+ "/" + context.LAMISPlus_Database + "?" + context.LAMISPlus_AdditionalParams;
				...
				try {
					...
					while (rs_tDBInput_17.next()) {
						...
						if (colQtyInRs_tDBInput_17 < 1) {
							...
							row19.func_application_codeset = routines.system.JDBCUtil.getString(rs_tDBInput_17, 1,
									false);
						}
	...
	public void tDBInput_18Process(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				// source node:tDBInput_18 - inputs:() outputs:(row20,row20) | target
				// node:tAdvancedHash_row20 - inputs:(row20) outputs:()
				// linked node: tMap_6 - inputs:(row15,row20)
				...
				String url_tDBInput_18 = "jdbc:postgresql://" + context.LAMISPlus_Server + ":" + context.LAMISPlus_Port
						+ "/" + context.LAMISPlus_Database + "?" + context.LAMISPlus_AdditionalParams;
	...
	public void tDBInput_19Process(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				// source node:tDBInput_19 - inputs:(after_tDBInput_12) outputs:(row21,row21) |
				...
				String url_tDBInput_19 = "jdbc:postgresql://" + context.LAMISPlus_Server + ":" + context.LAMISPlus_Port
						+ "/" + context.LAMISPlus_Database + "?" + context.LAMISPlus_AdditionalParams;
				...
				try {
					...
					while (rs_tDBInput_19.next()) {
						...
						if (colQtyInRs_tDBInput_19 < 1) {
							...
							row21.adherence_application_codeset = routines.system.JDBCUtil.getString(rs_tDBInput_19, 1,
									false);
						}
	...
	public static void main(String[] args) {
		final clinic_followup_migration clinic_followup_migrationClass = new clinic_followup_migration();

		int exitCode = clinic_followup_migrationClass.runJobInTOS(args);
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = clinic_followup_migration.class.getClassLoader().getResourceAsStream(
					"lamis3_etl/clinic_followup_migration_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = clinic_followup_migration.class.getClassLoader()
		...
		if (false) {
			System.out.println((endUsedMemory - startUsedMemory)
					+ " bytes memory increase when running : clinic_followup_migration");
		}
...
/************************************************************************************************
 * 571579 characters generated by Talend Open Studio for Big Data on the May 31,
 * 2021 12:44:11 PM WAT
 ************************************************************************************************/