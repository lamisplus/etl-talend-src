				...
				try {
					...
					while (rs_tDBInput_4.next()) {
						...
						if (!rejectedInnerJoin_tMap_1) { // G_TM_M_020
							...
							row5HashKey.date_visit_start = row4.date_status == null ? null
									: new java.util.Date(row4.date_status.getTime());
...
// ###############################
				...
				fileName_tFileOutputDelimited_1 = (new java.io.File("client_status_not_migrated.csv")).getAbsolutePath()
						.replace("\\", "/");
				...
				try {
					...
					while (rs_tDBInput_6.next()) {
						...
						if (!rejectedInnerJoin_tMap_3) { // G_TM_M_020
							...
							row7HashKey.date_encounter = row6.date_status == null ? null
									: new java.util.Date(row6.date_status.getTime());
...
// Start of branch "form_data_to_create"
										if (form_data_to_create != null) {
											...
											JSONArray interuptn = new JSONArray(
													form_data_to_create.interuptn_application_codeset);
											JSONArray death = new JSONArray(
													form_data_to_create.death_application_codeset);
											JSONArray outcome = new JSONArray(
													form_data_to_create.tracking_outcome_application_codeset);

											JSONObject json = new JSONObject();
											json.put("date_hiv_current_status",
													form_data_to_create.date_status != null
															? formatter.format(form_data_to_create.date_status)
															: JSONObject.NULL);
											json.put("date_tracked",
													form_data_to_create.date_tracked != null
															? formatter.format(form_data_to_create.date_tracked)
															: JSONObject.NULL);
											json.put("date_agreed_return",
													form_data_to_create.agreed_date != null
															? formatter.format(form_data_to_create.agreed_date)
															: JSONObject.NULL);
											json.put("uuid", form_data_to_create.uuid);
											json.put("auto", form_data_to_create.auto);

											if (form_data_to_create.status != null
													&& form_data_to_create.status != "") {
												String status = form_data_to_create.status;
												status = status.equalsIgnoreCase("HIV_PLUS_NON_ART") ? "HIV+ non ART"
														: status;
												status = status.equalsIgnoreCase("ART_TRANSFER_IN") ? "ART Transfer In"
														: status;
												status = status.equalsIgnoreCase("HIV_EXPOSED_STATUS_UNKNOWN")
														? "HIV Exposed Status Unknown"
														: status;
												status = status.equalsIgnoreCase("STOPPED_TREATMENT")
														? "Stopped Treatment"
														: status;
												status = status.equalsIgnoreCase("PRE_ART_TRANSFER_OUT")
														? "Pre-ART Transfer Out"
														: status;
												status = status.equalsIgnoreCase("LOST_TO_FOLLOWUP")
														? "Lost to Follow Up"
														: status;

												status = status.equalsIgnoreCase("KNOWN_DEATH") ? "Died (Confirmed)"
														: status;
												status = status.equalsIgnoreCase("ART_TRANSFER_OUT")
														? "ART Transfer Out"
														: status;
												status = status.equalsIgnoreCase("ART_RESTART") ? "ART Restart"
														: status;
												status = status.equalsIgnoreCase("HIV_NEGATIVE") ? "HIV Negative"
														: status;
												status = status.equalsIgnoreCase("ART_START") ? "ART Start" : status;
												// status = status.equalsIgnoreCase("LOST_TO_FOLLOWUP") ? "HIV Exposed
											...
											if (form_data_to_create.reason_for_interruption != null
													&& form_data_to_create.reason_for_interruption != "") {
												String status = form_data_to_create.reason_for_interruption;

												JSONObject item = null;
												for (int x = 0; x < interuptn.length(); x++) {
													if (interuptn.getJSONObject(x).getString("display")
															.equalsIgnoreCase(status)) {
													...
													context.application_codeset_not_present = context.application_codeset_not_present
															+ (context.application_codeset_not_present != null
																	&& context.application_codeset_not_present != ""
																			? ",'"
																			: "'")
															+ "Reason For Interruption - " + status + "'";
												}

											}

											if (form_data_to_create.cause_of_death != null
													&& form_data_to_create.cause_of_death != "") {
												String status = form_data_to_create.cause_of_death;
												JSONObject item = null;
												for (int x = 0; x < death.length(); x++) {
													if (death.getJSONObject(x).getString("display")
															.equalsIgnoreCase(status)) {
											...
											if (form_data_to_create.outcome != null
													&& form_data_to_create.outcome != "") {
												String status = form_data_to_create.outcome;
												status = status.equalsIgnoreCase("TRACED_UNABLE_TO_LOCATE")
														? "Traced Patient (Unable to locate)"
														: status;
												status = status.equalsIgnoreCase("TRACED_AGREED_TO_RETURN_TO_CARE")
														? "Traced Patient and agreed to return to care"
														: status;
												JSONObject item = null;
												for (int x = 0; x < outcome.length(); x++) {
													if (outcome.getJSONObject(x).getString("display")
															.equalsIgnoreCase(status)) {
											...
											if (tHash_Lookup_row11 != null
												...
												// lookup 'row11' and it contains more one result from keys :
												// row11.encounter_id = '" + row11HashKey.encounter_id + "',
												// row11.backlog_uuid = '" + row11HashKey.backlog_uuid + "'");
											} // G 071
...
// ###############################      
	...
	public void tJava_6Process(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				System.out.println(
						"*******************CLIENT STATUS MIGRATION COMPLETED*********************************");
				...
				try {
					...
					while (rs_tDBInput_2.next()) {
						...
						if (colQtyInRs_tDBInput_2 < 1) {
							...
							row2.tracking_outcome_application_codeset = routines.system.JDBCUtil
				...
				try {
					...
					while (rs_tDBInput_8.next()) {
						...
						if (colQtyInRs_tDBInput_8 < 1) {
							...
							row8.status_application_codeset = routines.system.JDBCUtil.getString(rs_tDBInput_8, 1,
									false);
						}
				...
				try {
					...
					while (rs_tDBInput_9.next()) {
						...
						if (colQtyInRs_tDBInput_9 < 1) {
							...
							row9.interuptn_application_codeset = routines.system.JDBCUtil.getString(rs_tDBInput_9, 1,
									false);
						}
				...
				try {
					...
					while (rs_tDBInput_11.next()) {
						...
						if (colQtyInRs_tDBInput_11 < 1) {
							...
							row12.death_application_codeset = routines.system.JDBCUtil.getString(rs_tDBInput_11, 1,
									false);
						}
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = client_status_migration.class.getClassLoader().getResourceAsStream(
					"lamis3_etl/client_status_migration_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = client_status_migration.class.getClassLoader()
		...
		if (false) {
			System.out.println((endUsedMemory - startUsedMemory)
					+ " bytes memory increase when running : client_status_migration");
		}
...
/************************************************************************************************
 * 404746 characters generated by Talend Open Studio for Big Data on the May 31,
 * 2021 12:45:20 PM WAT
 ************************************************************************************************/